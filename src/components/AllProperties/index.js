...
const samplePropertiesData = [
  {
    id: uuidv4(),
    image:
      "https://img.freepik.com/premium-photo/house-with-large-porch-large-window-with-sun-shining-through_1260025-42919.jpg?w=740",
    title: "Luxary Rooms",
  ...
  },
  {
    id: uuidv4(),
    image:
      "https://img.freepik.com/free-photo/3d-rendering-beautiful-luxury-bedroom-suite-hotel-with-tv_105762-2301.jpg?t=st=1722415633~exp=1722419233~hmac=e18d9a9d8cab21eaa68cc9fddfbeace0a498463a248678f424d53ea6dad8452a&w=900",
    title: "Family Rooms",
  ...
  },
  {
    id: uuidv4(),
    image:
      "https://img.freepik.com/premium-photo/tropical-villa-kerala_1015255-74133.jpg?w=1060",
  ...
  },
  {
    id: uuidv4(),
    image:
      "https://img.freepik.com/premium-photo/exterior-view-empress-market_1029622-25016.jpg?w=740",
    title: "Bachelor's Rooms",
  ...
  },
  {
    id: uuidv4(),
    image:
      "https://img.freepik.com/premium-photo/indian-style-luxury-decorated-house-with-ariel-view-with-big-lawn_876876-2349.jpg?w=740",
        ...
        <CartContext.Consumer>
          {(value) => {
            ...
            const renderProductsListView = () => {
              const { productsList, locationOptions, priceOptions, bedRooms } =
                this.state;
              const shouldShowProductsList = productsList.length > 0;

              const dataPropertiesList = productsList.filter(
                (each) =>
                  each.description.includes(locationOptions) ||
                  each.price <= priceOptions ||
                  each.bedroomsNo === bedRooms.bedroomsNo
              );

              const onClickAddToCart = (event) => {
                const cartItemData = productsList.filter(
                  ...
                  <ul className="products-list">
                    {dataPropertiesList.map((product) => (
                      <li className="product-item" key={product.id}>
                        ...
                        <p className="price">
                          Number of BedRooms: {product.bedroomsNo}/-
                        </p>
                        <button
                          className="buttn"
            ...
            return (
              <>
                <div className="background-cart-feature">
                  <Link to="/cart" className="link-style">
                    <p className="cart-text">Cart{renderCartItemsCount()}</p>
                  </Link>
                </div>
                  ...
                  <div className="fiter-section">
                    <select className="select-input" onChange={changeLocation}>
                      {locationOptions.map((each) => (
                        <option key={each.location} id={each.location}>
                          {each.location}
                        </option>
                      ))}
                    </select>
                    <select className="select-input" onChange={changePrice}>
                      {priceOptions.map((each) => (
                        <option key={each.price} id={each.price}>
                          {each.price}
                        </option>
                      ))}
                    </select>
                    <select className="select-input" onChange={changeBedRooms}>
                      {bedRooms.map((each) => (
                        <option key={each.bedroomsNo} id={each.bedroomsNo}>
                          {each.displayText}
                        </option>
                      ))}
                    </select>
